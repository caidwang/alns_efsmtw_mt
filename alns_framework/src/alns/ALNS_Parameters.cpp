	...
	if (loadOkay)
	{
		...
		if ( root )
		{
			...
			if(paramALNS)
			{
                ...
                TiXmlElement* saveFreq = paramALNS->FirstChildElement( "SaveFrequency" );
				if (saveFreq) {
				    stringstream str;
				    str << saveFreq->GetText();
				    str >> this->SaveFrequency;
				}
			}
            TiXmlElement* paramPenalty = root->FirstChildElement( "Penalty" );
			if (paramPenalty) {
			    TiXmlElement* vPenalty = paramPenalty->FirstChildElement("volumePenalty");
			    if (vPenalty) {
			        stringstream str;
			        str << vPenalty->GetText();
			        str >> this->volumePenalty;
			    }
                TiXmlElement* wPenalty = paramPenalty->FirstChildElement("weightPenalty");
                if (wPenalty) {
                    stringstream str;
                    str << wPenalty->GetText();
                    str >> this->weightPenalty;
                }
                TiXmlElement* tPenalty = paramPenalty->FirstChildElement("timeWinPenalty");
                if (tPenalty) {
                    stringstream str;
                    str << tPenalty->GetText();
                    str >> this->timeWinPenalty;
                }
                TiXmlElement* ePenalty = paramPenalty->FirstChildElement("energyPenalty");
                if (ePenalty) {
                    stringstream str;
                    str << ePenalty->GetText();
                    str >> this->energyPenalty;
                }
			}
		}
	}
...
ALNS_Parameters::ALNS_Parameters(ALNS_Parameters& p)
{
	...
	minimumWeight = p.minimumWeight; // The minimum possible weight for an operator.
